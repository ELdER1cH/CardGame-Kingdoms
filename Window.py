...
class Window(main_chat.Window):
  def __init__(self,*args,**kwargs):
    ...
    self.batch_gameState = Batch.CardBatch()
    self.batch_startState = Batch.StartscreenBatch()
  ...
  def select(self,target,clicked_card):
    if target.special_tag != "immovable" or target.y == 0:
      if target.owner == self.batch_gameState.castle.owner:
        self.batch_gameState.select_card(target)
      else:
  ...
  def on_mouse_press(self,x,y,button,MOD):
    ...
    if button == mouse.LEFT:
      if self.state_of_game == 'start':
        if x < self.width//2 + 40 and x >  self.width//2 - 40 and y > self.height//2 - 20 and y < self.height//2 + 20:
          self.state_of_game = 'game'
          
      
      if self.state_of_game == 'game':
        ###NEW CLICK/ TARGET
        target = self.batch_gameState.get_card((x,y))
        if target == None: return
        ###OLD CLICK/ SELECT
        clicked_card = self.batch_gameState.get_card(self.batch_gameState.select_frame.position)
        ...
        if clicked_card != None:
          ...
          if target == clicked_card:
            self.batch_gameState.hide(self.batch_gameState.select_frame)
            ...
            if target.y > 0:
              ...
              if target.owner == clicked_card.owner:
                ...
                if target.special_tag == "unoccupied_field":
                  ...
                  if self.batch_gameState.castle.mana >= clicked_card.price:
                    self.batch_gameState.castle.mana -= clicked_card.price
                    ...
                    self.batch_gameState.update_hand(target)
                    self.batch_gameState.hide(self.batch_gameState.select_frame)
                    #UPDATE STATS DISPLAY TO SHOW RIGHT MANA AMOUT
                    self.batch_gameState.update_disp(clicked_card)
                  else:
          ...
          #---NOT HAND---
            ...
            adjacent = self.batch_gameState.get_adjacent(clicked_card.position)
        ...
        ##SINCE THERES NO SELECTED CARD, TARGET = NEW SELECT
          ...
          if target.y > 0 and target.y < 800:
            self.batch_gameState.disp.update_to_enemy(target)
  ...
  def on_draw(self):
    self.clear()
    if self.state_of_game == 'start':
      self.batch_startState.draw()
      
    elif self.state_of_game == 'game':
      self.batch_gameState.draw()
    else:
      self.batch_gameState.draw()
    self.pop_up.draw()
  ...
  def on_cmd(self,cmd):
        if cmd[0] == '/hello world':
            ...
            if len(cmd) == 2:
                if cmd[1] == "-me":
                    self.batch_gameState.castle.mana = 99999
                else:
                    try:
                        self.batch_gameState.castle.mana += int(cmd[1])
            ...
            if len(cmd) >= 2 and cmd[1] == "-me":
              self.batch_gameState.castle.health = 99999
            elif len(cmd) == 4:
              try:
                card = self.batch_gameState.get_card((int(cmd[1])*120,int(cmd[2])*100))
              except ValueError as err:
                self.g_print("§cplease enter an integer as an argument! %s" % (err))
              if card != None:
